...
export default function RecommendationProfileCard({ source = 'home' }) {
  ...
  if (source === 'discovery') {
    const DiscoveryProfileSettings = require('./DiscoveryProfileSettings').default;
    return manager && state ? <DiscoveryProfileSettings key={`${scope}:${source}`} manager={manager} state={state} ready={libraryReady} /> : null;
  }
