...
const AAD=Buffer.from('biu-video-v2');
...
export function seal(library,key,nonce,device,parents=[]) {
  ...
  requireValue(library?.version===1,'音乐库数据格式无效');
  ...
  requireValue(payload.length>=192,'加密快照无效');
...
export function unseal(payload,key,expected) {
  requireValue(payload.length>=192&&Buffer.from(payload.subarray(0,4)).toString()==='BIU2','无效加密快照');
  ...
  requireValue(length>0,'压缩数据长度无效');
